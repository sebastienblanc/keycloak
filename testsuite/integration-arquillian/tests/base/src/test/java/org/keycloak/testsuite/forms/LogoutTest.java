...
public class LogoutTest extends AbstractTestRealmKeycloakTest {
    ...
    public void logoutRedirect() {
        ...
        String redirectUri = oauth.getRedirectUri() + "?logout";
    ...
    public void logoutMultipleSessions() throws IOException {
        ...
        driver.navigate().to(oauth.getLogoutUrl().redirectUri(oauth.getRedirectUri()).build());
        events.expectLogout(sessionId).detail(Details.REDIRECT_URI, oauth.getRedirectUri()).assertEvent();
        ...
        driver.navigate().to(oauth.getLogoutUrl().redirectUri(oauth.getRedirectUri()).build());
        events.expectLogout(sessionId3).detail(Details.REDIRECT_URI, oauth.getRedirectUri()).assertEvent();
    }
